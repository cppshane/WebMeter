(function(){
  ...
  function createMesh() {
    ...
    material = new FSS.Material(MESH.meshAmbient, MESH.meshDiffuse);
  ...
  function createLights() {
    ...
    for (l = 0; l < LIGHT.count; l++) {
      light = new FSS.Light(LIGHT.lightAmbient, LIGHT.lightDiffuse);
  ...
  function addControls() {
    ...
    // Add Mesh Controls
    controller = meshFolder.addColor(MESH, 'meshAmbient');
    controller.onChange(function(value) {
      for (i = 0, l = scene.meshes.length; i < l; i++) {
        scene.meshes[i].material.ambient.set(value);
      }
    });
    controller = meshFolder.addColor(MESH, 'meshDiffuse');
    controller.onChange(function(value) {
      for (i = 0, l = scene.meshes.length; i < l; i++) {
    ...
    controller = lightFolder.addColor(LIGHT, 'lightAmbient');
    controller.onChange(function(value) {
    ...
    controller = lightFolder.addColor(LIGHT, 'lightDiffuse');
    controller.onChange(function(value) {
      for (i = 0, l = scene.lights.length; i < l; i++) {
        light = scene.lights[i];
